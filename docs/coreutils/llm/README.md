...
> [!IMPORTANT]
> **The honest output is the event stream — `llm` never folds.** The device emits a stream of `ChatEvent`s (typed `text_start`/`text_delta`/…/`complete` frames); `llm` passes them through. It does **not** assemble them into a `ChatMessage` — that is a *reduce* over the stream, and it lives downstream (`chat-codec fold`), composed via pipe. This is the law of the family: the device's raw output is the raw output; sugar above it is a separate composable block. `llm` never fuses a transformer into itself, and never invents plumbing the shell already does.

> [!TIP]
> `llm` has two registers, same tool, same device — the flags pick the projection of the stream. **Casual**: `llm "…"` and `llm chat` — text out, the answer streaming to your terminal. **Scriptable**: `… | llm --output-format typed` — the structured `ChatEvent` stream, one frame per record, a Unix filter you compose a turn-loop out of.
...
`/exit` or `Ctrl-D` ends the session. Nothing is written to disk — when the process exits, the conversation is gone. `llm` keeps no history; BentOS does not persist your sessions behind your back. (Persistent, forkable conversation is `chat` over `tx`, a different coreutil — `llm chat` is the inert, in-memory convenience.) `chat` accepts the same `-d` / `-s` flags as the single-shot form.
...
| `--input-encoding <protobuf\|json>` | `inputEncoding` | Honoured when `--input-format typed`. `protobuf` (default): length-prefix framed binary. `json`: proto3-JSON. Ignored under `text` format. |
| `--output-encoding <protobuf\|json>` | `outputEncoding` | Honoured when `--output-format typed`. `protobuf` (default): length-prefix framed binary. `json`: proto3-JSON. Ignored under `text` format. |
...
> [!NOTE]
> **Encoding is serialization, not framing.** `json` names the proto3-JSON *serialization* of each record — nothing more. The newline that separates one record from the next on a byte-stream — the thing that makes a `.jsonl` file — is *framing*, the I/O syscall boundary projected onto the pipe. The device speaks records (one `read()` is one `ChatEvent`, datagram, no in-band delimiter); the coreutil delimits them with a newline when it flattens that datagram stream onto `stdout`. So `messages.jsonl` is **`json`-encoded, newline-framed**: the flag carries the encoding, the filename carries the framing. They are orthogonal, and the encoding axis names only the former.

> [!NOTE]
> **`--output-format typed` emits events, not a message.** You get the wire of [chatinference-subsystem.md](../../../../../hq/workshop/bentos/chatinference-subsystem.md) — one canonical proto3-JSON `ChatEvent` (a `oneof`, oneof-keyed and camelCase) per record:
>
> ```
> {"textStart":{"index":0}}
> {"textDelta":{"index":0,"text":"Hello"}}
> {"textStop":{"index":0}}
> {"complete":{"metadata":{"model":"claude-sonnet-4","stopReason":"CHAT_STOP_REASON_END_TURN","usage":{"inputTokens":"150","outputTokens":"42"}}}}
> ```
>
> A consumer that matches only `block` and `complete` works with or without streaming; the triads are the extra capability streaming switches on. To collapse the stream into the assembled `ChatMessage`, fold it downstream: `… | chat-codec fold`.
...
```sh
cat messages.jsonl | llm --input-format typed --input-encoding json \
  --output-format typed --output-encoding json \
  | chat-codec fold >> messages.jsonl
...
cat messages.jsonl | llm --input-format typed --input-encoding json \
  --output-format typed --output-encoding json \
  | tee >(chat-render) | chat-codec fold >> messages.jsonl
...
while true; do
  cat messages.jsonl | llm --input-format typed --input-encoding json \
    --output-format typed --output-encoding json \
    --function weather.json | chat-codec fold >> messages.jsonl
