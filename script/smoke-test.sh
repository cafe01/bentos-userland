...
#
# The floor is a real release old enough that `update` always has somewhere
# to move from — pinned by name, never "whatever the previous release was".
# `update` itself still resolves the true latest the one way production ever
# does; this script never guesses what that is. Caveat named, not solved
# here: the pin works by tag-prefix startsWith, which today's tags (v0.1.1,
# v0.1.2, v0.1.3) make exact, but v0.1.10 would also start with "v0.1.1" —
# the day a tag like that is cut, this floor needs a prefix that cannot
# collide (or the mechanism it leans on needs an exact-match option).
...
step "self-update: only bentos may move"
...
step "update: the rest of the set, self left alone"
...
for n in $others; do
  echo "$moved" | grep -qx "$n" || fail "update never moved $n off the floor"
done
assert_report_matches_bytes "$report" "bentos" "$others"
assert_executes

step "rollback: the whole set, back to the floor's own bytes"
