// Anthropic driver — conformance suite, run at the AGGREGATOR (userland).
//
...
//
// Proves the Anthropic driver honors the ChatInference contract by running the
...
//
// NOTE: this harness binds the driver DIRECTLY (anthropicChatDriver in fake mode)
// rather than via registerBundledLlmDrivers/bootLlmDevice — it needs the fake
// gate + error flags, which the production registry path deliberately does not
// expose. The bundled-registry path is exercised by boot_test.dart.
...
void main() {
  registerConformanceSuite(AnthropicConformanceHarness());
}

/// Wires the Anthropic driver (fake mode) into the conformance harness.
class AnthropicConformanceHarness implements ChatDriverHarness {
  @override
  ...
  @override
  bool get supportsReasoningBudget => true;

  @override
  String devicePath(String model) => '/dev/llm/anthropic/$model';
  ...
  // Anthropic's wire format natively handles N results in one message (loop
  // over content blocks), so the default single-call fake already satisfies
  // the parallel group. No override needed — delegates to device(model).
  @override
  ChatDevice parallelFunctionCallDevice(String model) => device(model);
