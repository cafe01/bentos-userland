...
void main() {
  group('MemStore — layout, cascade, shadow, write-target', () {
    ...
    MemStore storeAt(String cwd) {
      final vantage = hab.resolver.enclosing(cwd);
      return MemStore(
        vantage: vantage,
        entity: hab.entity,
        fs: hab.fs,
        writer: MemWriter(hab.fs, hab.now),
      );
    }

    setUp(() => hab = MemHabitat());
    ...
    test('replace of an inherited topic rewrites the ancestor in place', () {
      ...
      expect(store.readAt(ancestor, 'founders')!.body, 'new');
    });

    test('missing store lists empty, does not crash', () {
      hab.place('/hq/cto');
      expect(storeAt('/hq/cto').listAt(storeAt('/hq/cto').vantage), isEmpty);
    });
  });
}
