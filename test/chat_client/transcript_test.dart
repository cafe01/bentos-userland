...
void main() {
  group('Transcript', () {
    ...
    test('a topic line is not a message and never anchors the mark', () {
      final t = Transcript();
      t.append(SpokenLine(_msg('a')));
      t.append(TopicLine('new topic', const Handle('cafe', ''), DateTime(2026, 1, 1)));
      t.markRead();

      expect(t.readMark, 'a');
    });

    test('a reader scrolled up stays put as new lines land', () {
      final t = Transcript();
      t.append(SpokenLine(_msg('a')));
      t.append(SpokenLine(_msg('b')));
      t.scrollUp(1);
      expect(t.isAtBottom, isFalse);
      expect(t.scrollFromBottom, 1);

      t.append(SpokenLine(_msg('c')));
      expect(t.scrollFromBottom, 2);
      expect(t.isAtBottom, isFalse);
    });

    test('a reader at the bottom keeps following as new lines land', () {
      final t = Transcript();
      t.append(SpokenLine(_msg('a')));
      expect(t.isAtBottom, isTrue);

      t.append(SpokenLine(_msg('b')));
      expect(t.isAtBottom, isTrue);
    });

    test('scrollToBottom snaps back to following', () {
      final t = Transcript();
      t.append(SpokenLine(_msg('a')));
      t.append(SpokenLine(_msg('b')));
      t.scrollUp(5);
    ...
    test('scroll clamps to the available lines', () {
      final t = Transcript();
      t.append(SpokenLine(_msg('a')));
      t.scrollUp(50);
      expect(t.scrollFromBottom, 1);
    ...
    test('reading lines touches nothing — a render pays only for the window it slices', () {
    ...
    test('unreadTail is bounded by the unread tail, never by total history', () {
      final backing = _CountingLines(<TranscriptLine>[]);
      final t = Transcript(backing: backing);
    ...
    test('a caller needing both count and boundary pays for one pass, not two', () {
      final backing = _CountingLines(<TranscriptLine>[]);
      final t = Transcript(backing: backing);
      for (var i = 0; i < 20000; i++) {
        t.append(SpokenLine(_msg('m$i')));
