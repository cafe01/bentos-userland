/// `chat-data validate` — validate a ChatEvent JSONL stream from stdin.
///
...
class ValidateCommand extends Command<int> {
  ...
  @override
  String get invocation => 'llm … | chat-data validate';
  ...
  Future<int> run() async {
    ...
    await for (final line in stdin
      ...
      try {
        ...
        stderr.writeln('chat-data validate: line $lineNum: $e');
        errors++;
      }
    }

    if (errors > 0) {
      stderr.writeln('chat-data validate: $errors error(s) in $lineNum line(s)');
      return 1;
    }
    return 0;
  }
