...
| `unknown-option` | every verb | an option was typed that the verb does not declare | `no option --<typed>. Did you mean --<nearest>?` |
...
- **Nearest by name** is prefix and substring matching, cap 3, no fuzzy-match dependency. One implementation serves topics and option names both.
